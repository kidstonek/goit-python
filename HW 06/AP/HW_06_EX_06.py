...
Мы продолжим работать в этом направлении и создадим функцию, которая будет искать рецепт в файле
и возвращать найденный рецепт в виде словаря определенной формы.
...
Каждая запись начинается с первичного ключа базы данных MongoDB.
Дальше, через запятую, идет название рецепта, а после, через запятую, идет перечисление ингредиентов рецепта.
...
def get_recipe(path, search_id):
    ...
    try:
        with open(path, 'r') as fh:
            for i in fh:
                #print(i)
                if i.find(search_id) == 0:
                    recipe["id"] = i.split(',')[0]
                    recipe["name"] = i.split(',')[1]
                    ingridients.append(i.split(',')[2])
                    ingridients.append(i.split(',')[3])
                    tmp_ing = i.split(',')[4]
                    ingridients.append(tmp_ing.strip('\n'))
